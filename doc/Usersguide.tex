...
ratio testing (LRT) and m-fold cross validation.  The m-fold cross validation can only be applied to ``envseq'' and ``xenvpls'', while AIC, BIC and LRT can be applied to ``env'', ``henv'', ``ienv'', ``penv'', ``senv'' and ``xenv'', except that LRT cannot be applied to ``senv'',
...
``ienv'', ``penv'', ``senv'' and ``xenv'', and the choices for \texttt{mfoldcv} are ``envseq'' and ``xenvpls''.  The output is the dimension
...
\end{Verbatim}
respectively. For the inputs, modelType in all three functions can be ``env'', ``henv'', ``ienv'', ``penv'', ``senv'' and ``xenv'', while modelType in \texttt{bootstrapse} can also be ``envseq'' and ``xenvpls''; ModelOutput in \texttt{prediction}
...
$\Omegabf$, $\Omegabf_{0}$ and $\alphabf$, and also the number of parameters in the model and number of observations in the data.  It does not contain any information based on the likelihood function, for example, the maximized log-likelihood, the asymptotic covariance matrix of $\vecc(\betabf)$, the asymptotic standard errors of elements in $\betabf$, etc.  This is because \texttt{envseq} is mainly implemented for small sample size cases, under which the likelihood cannot be estimated.  Then any inference that is based on the covariance matrix of $\vecc(\betabf)$ such as estimation, prediction and test coefficients cannot be performed with ``envseq''.  But we can still estimate the bootstrap standard errors for $\betabfhat$ by 
\begin{Verbatim}[commandchars=\\\{\}]
...
Another utility of \texttt{envseq} is that it can provide a starting value for \texttt{env} when $r$ is large.  \texttt{env} can be very slow if $r$ is large, while \texttt{envseq} is faster than \texttt{env}.  Using the starting value from \texttt{envseq} can save much computing time, and at the same time, may still have the good performance from \texttt{env}.  An example is shown as follows.
\begin{Verbatim}[commandchars=\\\{\}]
load wheatprotein.txt
X = wheatprotein(:, 8);
Y = wheatprotein(:, 1 : 6);
