...
\item[{envpls}] Envelope Model Using Sequential Algorithm Module.  This module also implements the envelope model, but uses a sequential computing algorithm.  This algorithm makes the envelope model applicable for small sample size cases \citep{cook2012envelopes}.
\item[{henv}] Heteroscedastic Envelope Module. The heteroscedastic envelope model is used when the data has none constant error structure \citep{su2012estimation}.
...
is relaunched.  If a previous version of the toolbox is installed, it should be removed before installing the new version.  To remove, simply delete the folder of the toolbox.
...
ratio testing (LRT) and m-fold cross validation.  The m-fold cross validation can only be applied to ``envpls'' and ``xenvpls'', while AIC, BIC and LRT can be applied to ``env'', ``henv'', ``ienv'', ``penv'', ``senv'' and ``xenv'', except that LRT cannot be applied to ``senv'',
...
The syntax of AIC, BIC, LRT and m-fold cross validation are 
\begin{Verbatim}[commandchars=\\\{\}]
...
a string indicting the model. The choices for \texttt{modelselectaic}, \texttt{modelselectbic} and \texttt{modelselectlrt} can be ``env'', ``henv'',
``ienv'', ``penv'', ``senv'' and ``xenv'', and the choices for \texttt{mfoldcv} are ``envpls'' and ``xenvpls''.  The output is the dimension
...
\end{Verbatim}
The input X and Y are the same as the input in the dimension selection
...
\texttt{envpls} and \texttt{xenvpls} fit the same model as \texttt{env} and \texttt{xenv}, respectively, but use a different computing algorithm, making them capable to accommodate small sample size cases. With large sample size cases, \texttt{envpls} and \texttt{xenvpls} can also be applied, and usually give different results from \texttt{env} and \texttt{xenv}.  \citep{cook2012envelopes} indicates that \texttt{env} and \texttt{xenv} normally give better results, and therefore are recommended.
...
\end{Verbatim}
respectively. For the inputs, modelType in all three functions can be ``env'', ``henv'', ``ienv'', ``penv'', ``senv'' and ``xenv'', while modelType in \texttt{bootstrapse} can also be ``envpls'' and ``xenvpls''; ModelOutput in \texttt{prediction}
...
$\Omegabf$, $\Omegabf_{0}$ and $\alphabf$, and also the number of parameters in the model and number of observations in the data.  It does not contain any information based on the likelihood function, for example, the maximized log-likelihood, the asymptotic covariance matrix of $\vecc(\betabf)$, the asymptotic standard errors of elements in $\betabf$, etc.  This is because \texttt{envpls} is mainly implemented for small sample size cases, under which the likelihood cannot be estimated.  Then any inference that is based on the covariance matrix of $\vecc(\betabf)$ such as estimation, prediction and test coefficients cannot be performed with ``envpls''.  But we can still estimate the bootstrap standard errors for $\betabfhat$ by 
\begin{Verbatim}[commandchars=\\\{\}]
bootse = bootstrapse(X, Y, u, B, 'envpls')
\end{Verbatim}

{\color{blue} To Dennis: I am not sure if we would like to include this in.  Using the sequential algorithm as the starting value normally gives a different result, which is often the local maximum.}


Another utility of \texttt{envpls} is that it can provide a starting value for \texttt{env} when $r$ is large.  \texttt{env} can be very slow if $r$ is large, while \texttt{envpls} is faster than \texttt{env}.  Using the starting value from \texttt{envpls} can save much computing time, and at the same time, may still have the good performance from \texttt{env}.  An example is shown as follows.
\begin{Verbatim}[commandchars=\\\{\}]
load wheatprotein.txt
X = wheatprotein(:, 8);
Y = wheatprotein(:, 1 : 6);
...
$\Omegabf$, $\Omegabf_{0}$ and $\mubf$, and also the number of parameters in the model and number of observations in the data.  It does not contain any information based on the likelihood function, for example, the maximized log-likelihood, the asymptotic covariance matrix of $\vecc(\betabf)$, the asymptotic standard errors of elements in $\betabf$, etc.  This is because \texttt{envpls} is mainly implemented for small sample size cases, under which the likelihood cannot be estimated.  The estimator is obtained by partial least squares algorithm.  We can check this by
\begin{Verbatim}[commandchars=\\\{\}]
ModelOutput = xenvpls(X, Y, u);
[XL, YL, XS, YS, BETA, PCTVAR, MSE, stats]  =  plsregress(X, Y, u);
subspace(ModelOutput.Gamma, stats.W)
\end{Verbatim}
The function \texttt{plsregress} is built in the Statistics toolbox in MATLAB, it does the partial least squares regression to the data.  The function \texttt{subspace} computes the angles between the two subspaces, if the angle is $0$, then the two subspaces are the same.  

For inference, any inference that is based on the covariance matrix of $\vecc(\betabf)$ such as estimation, prediction and test coefficients cannot be performed with ``xenvpls''.  But we can still estimate the bootstrap standard errors for $\betabfhat$ by 
\begin{Verbatim}[commandchars=\\\{\}]
...
The package sg\_min 2.4.1 by Ross Lippert \url{http://web.mit.edu/~ripper/}
