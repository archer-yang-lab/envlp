...
\begin{description}
\item[{tools}] Functions for dimension selection and inference.
\item[{env}] Functions that implement the envelope model. 
\item[{envseq}] Functions that implement the envelope model using sequential algorithm.
\item[{henv}] Functions that implement the heteroscedastic envelope model. 
\item[{ienv}] Functions that implement the inner envelope model. 
\item[{penv}] Functions that implement the partial envelope model. 
\item[{senv}] Functions that implement the scaled envelope model. 
\item[{xenv}] Functions that implement the envelope model in the predictor space. 
\item[{xenvpls}] Functions that implement the envelope model in the predictor space using partial least squares algorithm. 
\item[{envmean}] Functions that implement the envelope estimator of the multivariate mean.   
\item[{auxiliary}] Auxiliary functions used internally in the toolbox. 
\end{description}
...
\end{par} \vspace{1em}
\begin{par}
For model type for methods 'env', 'envpls', 'henv', 'ienv', ' senv', 'spls', ' sxenv', 'xenv' and 'xenvpls'. X is an n by p matrix, p is the number of predictors.
\end{par} \vspace{1em}
\begin{par}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{modelType}: A string of characters indicating the model, choices can be 'env', 'envpls', 'henv', 'ienv', 'penv', 'senv', 'spls', 'sxenv', 'xenv' and 'xenvpls'.
\end{par} \vspace{1em}
\begin{par}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.verbose: Flag to print out  the number of bootstrap samples, logical 0 or 1. Default value: 0.
\end{itemize}
...
\begin{par}
bootse: For 'env', 'envpls', 'henv', 'ienv', and 'senv', an r by p matrix containing the standard errors for elements in $\beta$ computed by bootstrap.  For 'penv', an r by p1 matrix containing the standard errors for $\beta_1$ computed by bootstrap.  For 'spls', 'sxenv', 'xenv' and 'xenvpls', a p by r matrix containing the standard errors for elements in $\beta$ computed by bootstrap.
\end{par} \vspace{1em}
...
\begin{verbatim}       SelectOutput = mfoldcv(X, Y, m, modelType)
       SelectOutput = mfoldcv(X, Y, m, modelType, Opts)\end{verbatim}
...
\end{par} \vspace{1em}
\begin{par}
For model type 'env', 'envseq', 'henv', 'ienv', 'senv', 'spls', 'sxenv', 'xenv' and 'xenvpls', X is an n by p matrix, p is the number of predictors.
\end{par} \vspace{1em}
\begin{par}
For model type 'penv', X is  A list containing the value of X1 and X2.
\end{par} \vspace{1em}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{modelType}: A string of characters indicating the model, choices can be 'env', 'envseq', 'henv', 'ienv', 'penv', 'senv', 'spls', 'sxenv', 'xenv' or 'xenvpls'.
\end{par} \vspace{1em}
\begin{par}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item Opts.verbose: Flag to print out  dimension selection process, logical 0 or 1. Default value: 0.
   \item Opts.table: Flag to tabulate the results, which contains cross validation error for each u.  Logical 0 or 1. Default value: 0.
   \item Opts.perm: A positive integer indicating number permutations of the observations, m-fold cross validation is run on each permutation. If not specified, the division is based on the sequential order of the observations.
   \item Opts.seed: A real number that set the seeds for permutations. Default value is 1.
\end{itemize}
...
\begin{par}
\textbf{SelectOutput}: A list containing the results of the selection.
\end{par} \vspace{1em}
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item SelectOutput.u: The dimension of the envelope subspace selected by m-fold cross validation.  An integer between 0 and r.
   \item SelectOutput.PreErr: A vector containing prediction errors for each u if Opts.perm is not specified, or a matrix with the element in the ith row and jth column containing the prediction error for u=j-1 and ith permutation of the observations.
\end{itemize}
...
\begin{verbatim}       load wheatprotein.txt
       ...
       m = 5;
       modelType = 'envpls';
       SelectOutput = mfoldcv(X, Y, m, modelType)\end{verbatim}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{modelType}: A string of characters indicating the model, choices can be 'env', 'henv', 'ienv', 'penv', 'senv', 'sxenv' and 'xenv'.
\end{par} \vspace{1em}
\begin{par}
\textbf{Opts}: A list containing the optional input parameters, to control the iterations in sg\_min. If one or several (even all) fields are not defined, the default settings are used.
\end{par} \vspace{1em}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.gradtol: Tolerance parameter for dF.  Default value: 1e-7.
   \item Opts.verbose: Flag to print out dimension selection process.  Logical 0 or 1. Default value: 0.
   \item Opts.table: Flag to tabulate the results, which contains AIC and log likelihood for each u. Logical 0 or 1. Default value: 0.
\end{itemize}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.gradtol: Tolerance parameter for dF.  Default value: 1e-7.
   \item Opts.verbose: Flag to print out dimension selection process.  Logical 0 or 1. Default value: 0.
   \item Opts.table: Flag to tabulate the results, which contains BIC and log likelihood for each u. Logical 0 or 1. Default value: 0.
\end{itemize}
...
\begin{par}
This function implements the Bayesian information criteria (BIC) to select the dimension of the envelope subspace for method 'env', 'henv', 'ienv', 'penv', 'senv', 'sxenv' and 'xenv'.
\end{par} \vspace{1em}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{modelType}: A string of characters indicating the model, choices can be 'env', 'henv', 'ienv', 'penv' and 'xenv'.
\end{par} \vspace{1em}
\begin{par}
\textbf{Opts}: A list containing the optional input parameters, to control the iterations in sg\_min. If one or several (even all) fields are not defined, the default settings are used.
\end{par} \vspace{1em}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.gradtol: Tolerance parameter for dF.  Default value: 1e-7.
   \item Opts.verbose: Flag to print out dimension selection process.  Logical 0 or 1. Default value: 0.
   \item Opts.table: Flag to tabulate the results, which contains log likelihood, test statistic, degrees of freedom and p-value for each test. Logical 0 or 1. Default value: 0.
\end{itemize}
...
* Xnew.X2 (only for 'penv'): A p2 by 1 vector containing the value of X2.\end{verbatim}
    \begin{par}
\textbf{infType}: A string of characters indicating the inference type, the choices can be 'estimation' or 'prediction'.
\end{par} \vspace{1em}
\begin{par}
\textbf{modelType}: A string of characters indicating the model, choices can be 'env', 'henv', 'ienv', 'penv', 'senv', 'sxenv' and 'xenv'.
\end{par} \vspace{1em}
...
\begin{par}
This function evaluates the user-specified model, could be 'env', 'henv', 'ienv', 'penv', 'senv', 'sxenv' or 'xenv', at new value Xnew.  It can perform estimation: find the fitted value when X = Xnew, or prediction: predict Y when X = Xnew.  The covariance matrix and the standard errors are also provided.
\end{par} \vspace{1em}
...
\begin{par}
\textbf{TestOutput}: A list containing test statistics, degrees of freedom for the reference chi-squared distribution, the p-value, and the covariance matrix of vec($L\hat{\beta} R$).  At the same time, a table is printed out.
\end{par} \vspace{1em}
\begin{itemize}
\setlength{\itemsep}{-1ex}
...
\begin{par}
This function tests for hypothesis $H_0: L\beta R = A$, versus $H_\alpha: L\beta R\neq A$.  The $\beta$ is estimated by a model in the envelope model.  If the user does not specify the values for L, R and A, then the test is equivalent to the standard F test on if $\beta = 0$ (for 'env', 'ienv', 'penv', 'senv', 'sxenv' and 'xenv'), or if the group main effects are all zeros (for 'henv').  The test statistics used is vec $(L\hat{\beta} R - A)$ $\hat{\Sigma}^{-1}$ vec $(L\hat{\beta} R - A)^{T}$, and the reference distribution is chi-squared distribution with degrees of freedom the same as the length of vec(A).
\end{par} \vspace{1em}
...
\rmfamily
\color{black} \section{aic\_predict2\_env}
...
\begin{verbatim}       u = aic_predict2_env(X, Y, Xnew)
       u = aic_predict2_env(X, Y, Xnew, Opts)\end{verbatim}
...
\begin{par}
\textbf{u}: Dimension of the constructed partial envelope. An integer between 0 and r.
\end{par} \vspace{1em}
...
\begin{enumerate}
\setlength{\itemsep}{-1ex}
   \item The codes are implemented based on the following reference: R.D. Cook (2013) ``Lecture Notes on Envelope Models and Methods.'' School of Statistics, University of Minnesota, Minneapolis.
\end{enumerate}
...
\begin{verbatim}       u = bic_predict2_env(X, Y, Xnew)
       u = bic_predict2_env(X, Y, Xnew, Opts)\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.ftol: Tolerance parameter for F.  Default value: 1e-10.
   \item Opts.gradtol: Tolerance parameter for dF.  Default value: 1e-7.
   \item Opts.verbose: Flag to print out  Grassmann manifold optimization process, logical 0 or 1. Default value: 0.
...
\begin{verbatim}       u = lrt_predict2_env(X, Y, alpha, Xnew)
       u = lrt_predict2_env(X, Y, alpha, Xnew, Opts)\end{verbatim}
...
\begin{verbatim}       SelectOutput = mfoldcv_env(X, Y, m)
       SelectOutput = mfoldcv_env(X, Y, m, Opts)\end{verbatim}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{Y}: Responses. An n by r matrix, r is the number of responses. The responses must be continuous variables.
\end{par} \vspace{1em}
\begin{par}
\textbf{m}: A positive integer that is used to indicate m-fold cross validation.
\end{par} \vspace{1em}
...
\rmfamily
\color{black}\section{predict2\_env}
...
\begin{verbatim}       PredictOutput = predict2_env(X, Y, u, Xnew, infType)\end{verbatim}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{u}: The dimension of the constructed partial envelope model.  An integer between from 0 to r.
\end{par} \vspace{1em}
\begin{par}
\textbf{Xnew}: The value of X with which to estimate or predict Y.  A p by 1 vector.
\end{par} \vspace{1em}
...
\begin{par}
This function evaluates the envelope model at new value Xnew.  It can perform estimation: find the fitted value when X = Xnew, or prediction: predict Y when X = Xnew.  The covariance matrix and the standard errors are also provided.  Compared to predict\_env, this function performs prediction through partial envelope model, which can be more accurate if the partial envelope is of smaller dimension and contains less variant material information.
\end{par} \vspace{1em}
...
\begin{par}
This function tests for hypothesis $H_0: L\beta R = A$, versus $H_\alpha: L\beta R\neq A$.  The $\beta$ is estimated by the envelope model.  If the user does not specify the values for L, R and A, then the test is equivalent to the standard F test on if $\beta = 0$.  The test statistics used is vec $(L\hat{\beta} R - A)$ $\hat{\Sigma}^{-1}$ vec $(L\hat{\beta} R - A)^{T}$, and the reference distribution is chi-squared distribution with degrees of freedom d1 * d2.
\end{par} \vspace{1em}
...
\setlength{\itemsep}{-1ex}
   ...
   \item ModelOutput.Gamma0: The orthogonal basis of the complement of the envelope subspace.  A p by p-u semi-orthogonal matrix.
...
\newpage
\rmfamily
\color{black}\section{mfoldcv\_envmean}
...
\begin{verbatim}       SelectOutput = mfoldcv_envmean(Y, m)
       SelectOutput = mfoldcv_envmean(Y, m, Opts)\end{verbatim}
...
\begin{verbatim}       SelectOutput = mfoldcv_envseq(X, Y, m)
       SelectOutput = mfoldcv_envseq(X, Y, m, Opts)\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item ModelOutput.eta: The coordinates of $\beta$ with respect to Gamma. A u by p matrix, the ith column contains the coordinates of the main effect of the ith group with respect to Gamma.
   \item ModelOutput.Omega: The coordinates of Sigma with respect to Gamma. A u by u by p matrix, ModelOutput.Omega(:, :, i) contains the coordinates of the covariance matrix of the ith group with respect to Gamma.
   \item ModelOutput.Omega0: The coordinates of Sigma with respect to Gamma0. An r - u by r - u matrix.
   \item ModelOutput.l: The maximized log likelihood function.  A real number.
...
\begin{enumerate}
\setlength{\itemsep}{-1ex}
   \item The codes are implemented based on the algorithm in Section 2.2 of Su and Cook (2013).
   \item The Grassmann manifold optimization step calls the package sg\_min 2.4.3 by Ross Lippert (http://web.mit.edu/$\sim$ripper/www/sgmin.html).
\end{enumerate}
...
\newpage
\rmfamily
\color{black}\section{mfoldcv\_henv}
...
\begin{verbatim}       SelectOutput = mfoldcv_henv(X, Y, m)
       SelectOutput = mfoldcv_henv(X, Y, m, Opts)\end{verbatim}
...
\begin{verbatim}       load wheatprotein.txt
       Opts.table = 1; % Print out the table of average prediction error for each u
       SelectOutput = mfoldcv_henv(X, Y, 5, Opts);\end{verbatim}
...
\end{verbatim} \rmfamily
\color{black}
\begin{verbatim}       Opts.perm = 10; % Run 5-fold CV on 10 permutations
       Opts.seed = 3; % Set seed for the permutations
       Opts.table = 1;
...
\begin{par}
This function tests for hypothesis $H_0: L\beta R = A$, versus $H_\alpha: L\beta R\neq A$.  The $\beta$ is estimated by the heteroscedastic envelope model.  If the user does not specify the values for L, R and A, then the test is equivalent to the standard F test on if all the main group effects are 0.  The test statistics used is vec $(L\hat{\beta} R - A)$ $\hat{\Sigma}^{-1}$ vec $(L\hat{\beta} R - A)^{T}$, and the reference distribution is chi-squared distribution with degrees of freedom d1 * d2.
\end{par} \vspace{1em}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{u}: Dimension of the inner envelope. An integer between 0 and p.
\end{par} \vspace{1em}
\begin{par}
\textbf{B}: Number of bootstrap samples.  A positive integer.
\end{par} \vspace{1em}
...
\setlength{\itemsep}{-1ex}
   ...
   \item ModelOutput.eta1: The transpose of the coordinates of $\beta$ with respect to Gamma1. A p by u matrix.
   \item ModelOutput.B: An (r - u) by (p - u) semi-orthogonal matrix, so that (Gamma, Gamma0 * B) spans $\beta$.
   \item ModelOutput.eta2: The transpose of the coordinates of $\beta$ with respect to Gamma0. A p by (p-u) matrix.
   \item ModelOutput.Omega1: The coordinates of Sigma with respect to Gamma1. A u by u matrix.
   \item ModelOutput.Omega0: The coordinates of Sigma with respect to Gamma0. An r - u by r - u matrix.
...
\rmfamily
\color{black}\section{mfoldcv\_ienv}
...
\begin{verbatim}       SelectOutput = mfoldcv_ienv(X, Y, m)
       SelectOutput = mfoldcv_ienv(X, Y, m, Opts)\end{verbatim}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item SelectOutput.u: The dimension of the inner envelope subspace selected by m-fold cross validation. An integer between 0 and p.
   \item SelectOutput.PreErr: A vector containing prediction errors for each u if Opts.perm is not specified, or a matrix with the element in the ith row and jth column containing the prediction error for u=j-1 and ith permutation of the observations.
\end{itemize}
...
\begin{verbatim}       load irisf.mat
       Opts.table = 1; % Print out the table of average prediction error for each u
       SelectOutput = mfoldcv_ienv(X, Y, 5, Opts);\end{verbatim}
...
\rmfamily
\color{black}\section{mfoldcv\_penv}
...
\begin{verbatim}       SelectOutput = mfoldcv_penv(X, Y, m)
       SelectOutput = mfoldcv_penv(X, Y, m, Opts)\end{verbatim}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item SelectOutput.u: The dimension of the partial envelope subspace selected by m-fold cross validation.  An integer between 0 and r.
   \item SelectOutput.PreErr: A vector containing prediction errors for each u if Opts.perm is not specified, or a matrix with the element in the ith row and jth column containing the prediction error for u=j-1 and ith permutation of the observations.
\end{itemize}
...
\begin{verbatim}       load fiberpaper.dat
       ...
       X.X2 = fiberpaper(:, 5 : 6);
       Opts.table = 1; % Print out the table of average prediction error for each u
       SelectOutput = mfoldcv_penv(X, Y, 5, Opts);\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item ModelOutput.eta: The coordinates of $\beta_1$ with respect to Gamma. A u by p1 matrix.
   \item ModelOutput.Omega: The coordinates of Sigma with respect to Gamma. A u by u matrix.
   \item ModelOutput.Omega0: The coordinates of Sigma with respect to Gamma0. An r - u by r - u matrix.
...
\begin{par}
\textbf{TestOutput}: A list containing test statistics, degrees of freedom for the reference chi-squared distribution, the p-value, and the covariance matrix of vec($L\hat{\beta}_{1} R$).  At the same time, a table is printed out.
\end{par} \vspace{1em}
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item TestOutput.chisqStatistic: The test statistics. A real number.
   \item TestOutput.df: The degrees of freedom of the reference chi-squared distribution.  A positive integer.
   \item TestOutput.pValue: p-value of the test.  A real number in [0, 1].
   \item TestOutput.covMatrix: The covariance matrix of vec($L\hat{\beta}_{1} R$). A d1 * d2 by d1 * d2 matrix.
\end{itemize}

\subsection*{Description}

\begin{par}
This function tests for hypothesis $H_0: L\beta_1 R = A$, versus $H_\alpha: L\beta_1 R\neq A$.  The $\beta_1$ is estimated by the partial envelope model.  If the user does not specify the values for L, R and A, then the test is equivalent to the standard F test on if $\beta_1 = 0$.  The test statistics used is vec$(L\hat{\beta}_{1} R - A)$ $\hat{\Sigma}^{-1}$ vec $(L\hat{\beta}_{1} R - A)^{T}$, and the reference distribution is chi-squared distribution with degrees of freedom d1 * d2.
\end{par} \vspace{1em}
...
\begin{par}
\textbf{u}: Dimension of the scaled envelope. An integer between 0 and r.
\end{par} \vspace{1em}
...
\rmfamily
\color{black}\section{mfoldcv\_senv}
...
\begin{verbatim}       SelectOutput = mfoldcv_senv(X, Y, m)
       SelectOutput = mfoldcv_senv(X, Y, m, Opts)\end{verbatim}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item SelectOutput.u: The dimension of the scaled envelope subspace selected by m-fold cross validation.  An integer between 0 and r.
   \item SelectOutput.PreErr: A vector containing prediction errors for each u if Opts.perm is not specified, or a matrix with the element in the ith row and jth column containing the prediction error for u=j-1 and ith permutation of the observations.
\end{itemize}
...
\begin{verbatim}       load('sales.txt')
       ...
       X = sales(:, 1 : 3);
       Opts.table = 1; % Print out the table of average prediction error for each u
       SelectOutput = mfoldcv_senv(X, Y, 5, Opts);\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.ftol: Tolerance parameter for F.  Default value: 1e-10.
   \item Opts.gradtol: Tolerance parameter for dF.  Default value: 1e-7.
   \item Opts.verbose: Flag to print out  number of iterations, logical 0 or 1. Default value: 0.
   \item Opts.rep: Number of replicates for scales. This option imposes special structure on scaling parameters. For example, if Opts.rep = [3 4], this means that the first three responses have the same scale and the next four responses share a different scale. The elements of this vector should sum to r. If not specified, the default is [], then all responses will be scaled differently. If all responses have the same scale, input [r], then the regular envelope will be applied to the data. The input should be a row vector.
\end{itemize}
...
\begin{enumerate}
\setlength{\itemsep}{-1ex}
   \item The codes are implemented based on the algorithm in Section 4.1 of Cook and Su (2013).
   \item The Grassmann manifold optimization step calls the package sg\_min 2.4.3 by Ross Lippert (http://web.mit.edu/$\sim$ripper/www/sgmin.html).
\end{enumerate}
...
\rmfamily
\color{black}\section{aic\_sxenv}
...
\begin{verbatim}       u = aic_sxenv(X, Y)
       u = aic_sxenv(X, Y, Opts)\end{verbatim}
    

\subsection*{Input}

\begin{par}
\textbf{X}: Predictors. An n by p matrix, p is the number of predictors and n is number of observations.  The predictors must be continuous variables.
\end{par} \vspace{1em}
\begin{par}
...
\begin{par}
\textbf{u}: Dimension of the scaled envelope. An integer between 0 and p.
\end{par} \vspace{1em}
...
\begin{verbatim}       load('chemo.mat')
       X = X(:, [6 11 21 22]);
       Opts.verbose = 1;
       Opts.table = 1;
       u = aic_sxenv(X, Y, Opts)\end{verbatim}
    
        \color{lightgray}\ttfamily \begin{verbatim}Current dimension 0
Current dimension 1
Current dimension 2
Current dimension 3
...
\rmfamily
\color{black}\section{bic\_sxenv}
...
\begin{verbatim}       u = bic_sxenv(X, Y)
       u = bic_sxenv(X, Y, Opts)\end{verbatim}
...
\newpage
\rmfamily
\color{black}\section{bstrp\_spls}
...
\begin{verbatim}       bootse = bstrp_spls(X, Y, u, B)
       bootse = bstrp_spls(X, Y, u, B, Opts)\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.rep: Number of replicates for scales. This option imposes special structure on scaling parameters. For example, if Opts.rep = [3 4], this means that the first three responses have the same scale and the next four responses share a different scale. The elements of this vector should sum to r. If not specified, the default is [], then all responses will be scaled differently. If all responses have the same scale, input [r], then the regular envelope will be applied to the data. The input should be a row vector.
   \item Opts.Gamma: The initial value for the envelope subspace. A p by u matrix. Default value is the one obtained from xenv, with $\Lambda^{-1}X$ being the predictor and Y being the response.
   \item Opts.Lambda: The initial value for the scales. A p by p diagonal matrix. Default value is the identity matrix.
\end{itemize}
...
\begin{verbatim}       load('chemo.mat')
       X = X(:, [6 11 21 22]);
       ModelOutput = spls(X, Y, 3);\end{verbatim}
    
\begin{verbatim}       Opts.Gamma = ModelOutput.Gamma;
       Opts.verbose = 1;
       B = 10;
       bootse = bstrp_spls(X, Y, 3, B, Opts)\end{verbatim}
    
        \color{lightgray}\ttfamily \begin{verbatim}Current number of bootstrap sample 1
...
\rmfamily
\color{black}\section{bstrp\_sxenv}
...
\begin{verbatim}       bootse = bstrp_sxenv(X, Y, u, B)
       bootse = bstrp_sxenv(X, Y, u, B, Opts)\end{verbatim}
...
\begin{verbatim}       load('chemo.mat')
       ...
       ModelOutput.asySE / sqrt(ModelOutput.n)\end{verbatim}
...
\begin{verbatim}       Opts.Gamma = ModelOutput.Gamma;
       Opts.verbose = 1;
       B = 10;
       bootse = bstrp_sxenv(X, Y, 3, B, Opts)\end{verbatim}
...
\begin{verbatim}       df = dF4sxenv(R, DataParameter)\end{verbatim}
...
\begin{verbatim}       f = F4sxenv(R, DataParameter)\end{verbatim}
...
\rmfamily
\color{black}\section{mfoldcv\_spls}
...
\begin{verbatim}       SelectOutput = mfoldcv_spls(X, Y, m)
       SelectOutput = mfoldcv_spls(X, Y, m, Opts)\end{verbatim}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item SelectOutput.u: The dimension of the scaled predictor envelope subspace selected by m-fold cross validation.  An integer between 0 and p.
   \item SelectOutput.PreErr: A vector containing prediction errors for each u if Opts.perm is not specified, or a matrix with the element in the ith row and jth column containing the prediction error for u=j-1 and ith permutation of the observations.
\end{itemize}
...
\newpage
\rmfamily
\color{black}\section{mfoldcv\_sxenv}
...
\begin{verbatim}       SelectOutput = mfoldcv_sxenv(X, Y, m)
       SelectOutput = mfoldcv_sxenv(X, Y, m, Opts)\end{verbatim}
...
\begin{verbatim}       load('chemo.mat')
       ...
       SelectOutput.u\end{verbatim}
        ...
        \color{lightgray}\ttfamily \begin{verbatim}Current dimension 1
Current dimension 2
Current dimension 3
Current dimension 4
Current dimension 5
...
\rmfamily
\color{black}\section{objfun\_spls}
...
\begin{verbatim}       f = objfun_spls(a, Gamma1, Gamma2, DataParameter)\end{verbatim}
    

\subsection*{Input}

\begin{par}
\textbf{a}: A scalar between 0 and 1 containing the initial value of a.
\end{par} \vspace{1em}
\begin{par}
\textbf{Gamma1}: A p by u matrix containing an orthogonal basis of the envelope subspace in the last iteration.
\end{par} \vspace{1em}
\begin{par}
\textbf{Gamma2}: A p by u matrix containing an orthogonal basis of the envelope subspace in the current iteration.
\end{par} \vspace{1em}
\begin{par}
\textbf{DataParameter}: A structure that contains the statistics calculated from the data.
\end{par} \vspace{1em}
...
\begin{par}
\textbf{f}: A scalar containing the value of the objective function evaluated at a.
\end{par} \vspace{1em}
...
\newpage
\rmfamily
\color{black}\section{objfun\_sxenv}
...
\begin{verbatim}       f = objfun_sxenv(d, Gamma, DataParameter)\end{verbatim}
    

\subsection*{Input}

\begin{par}
\textbf{d}: A (p - 1) by 1 vector containing the value of the scales (the second to the last diagonal element in $\Lambda$.
\end{par} \vspace{1em}
\begin{par}
\textbf{Gamma}: A p by u matrix containing an orthogonal basis of the envelope subspace.
\end{par} \vspace{1em}
\begin{par}
\textbf{DataParameter}: A structure that contains the statistics calculated from the data.
\end{par} \vspace{1em}
...
\rmfamily
\color{black}\section{predict\_sxenv}
...
\begin{verbatim}       PredictOutput = predict_sxenv(ModelOutput, Xnew, infType)\end{verbatim}
    

\subsection*{Input}

\begin{par}
\textbf{ModelOutput}: A list containing the maximum likelihood estimators and other statistics inherited from sxenv.
\end{par} \vspace{1em}
\begin{par}
\textbf{Xnew}: The value of X with which to estimate or predict Y.  A p by 1 vector.
\end{par} \vspace{1em}
...
\begin{par}
This function evaluates the scaled predictor envelope model at new value Xnew.  It can perform estimation: find the fitted value when X = Xnew, or prediction: predict Y when X = Xnew.  The covariance matrix and the standard errors are also provided.
\end{par} \vspace{1em}
...
\begin{verbatim}       ModelOutput = spls(X, Y, u)
       ModelOutput = spls(X, Y, u, Opts)\end{verbatim}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item ModelOutput.beta: The scaled predictor envelope estimator of the regression coefficients $\beta$. An p by r matrix.
   \item ModelOutput.SigmaX: The scaled envelope estimator of the covariance matrix of X, $\Sigma_X$.  A p by p matrix.
   \item ModelOutput.Lambda: The matrix of estimated scales. An p by p diagonal matrix with the first diagonal element equal to 1 and other diagonal elements being positive.
   \item ModelOutput.Gamma: The orthogonal basis of the envelope subspace. A p by u semi-orthogonal matrix.
   \item ModelOutput.Gamma0: The orthogonal basis of the complement of the envelope subspace.  A p by p - u semi-orthogonal matrix.
   \item ModelOutput.eta: The coordinates of $\beta$ with respect to Gamma. A u by r matrix.
   \item ModelOutput.Omega: The coordinates of Sigma with respect to Gamma. A u by u matrix.
   \item ModelOutput.Omega0: The coordinates of Sigma with respect to Gamma0. A p - u by p - u matrix.
   \item ModelOutput.muX: The estimated mean of the predictors in the scaled predictor envelope model.  A p by 1 vector.
   \item ModelOutput.muY: The estimated mean of the responses in the scaled predictor envelope model.  An r by 1 vector.
   \item ModelOutput.sigYcX: The estimated conditional covariance matrix of Y given X. An r by r matrix.
   \item ModelOutput.paramNum: The number of parameters in the scaled predictor envelope model.  A positive integer.
   \item ModelOutput.n: The number of observations in the data.  A positive integer.
\end{itemize}
...
\begin{verbatim}       load('chemo.mat')
       ...
       ModelOutput.Lambda\end{verbatim}
...
\begin{verbatim}       ModelOutput = sxenv(X, Y, u)
       ModelOutput = sxenv(X, Y, u, Opts)\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item ModelOutput.muY: The estimated mean of the responses in the scaled predictor envelope model.  An r by 1 vector.
   \item ModelOutput.sigYcX: The estimated conditional covariance matrix of Y given X. An r by r matrix.
   \item ModelOutput.covMatrix: The asymptotic covariance of vec($\beta$).  A pr by pr matrix.  The covariance matrix returned are asymptotic.  For the actual standard errors, multiply by 1 / n.
   \item ModelOutput.asySE: Asymptotic standard error for elements in $\beta$ under the scaled predictor envelope model.  A p by r matrix.  The standard errors returned are asymptotic, for actual standard errors, multiply by 1 / sqrt(n).
   \item ModelOutput.ratio: The asymptotic standard error ratio of the standard multivariate linear regression estimator over the scaled predictor envelope estimator, for each element in $\beta$.  A p by r matrix.
   \item ModelOutput.paramNum: The number of parameters in the scaled predictor envelope model.  A positive integer.
   \item ModelOutput.l: The maximized log likelihood function.  A real number.
   \item ModelOutput.n: The number of observations in the data.  A positive integer.
\end{itemize}
...
\begin{enumerate}
\setlength{\itemsep}{-1ex}
   \item The codes are implemented based on the algorithm in Section 2.2 of Cook and Su (2015).
   \item The Grassmann manifold optimization step calls the package sg\_min 2.4.3 by Ross Lippert (http://web.mit.edu/$\sim$ripper/www/sgmin.html).
\end{enumerate}
...
\rmfamily
\color{black}\section{testcoefficient\_sxenv}


\begin{verbatim}This function tests the null hypothesis L * beta * R = A versus the
alternative hypothesis L * beta * R ~= A, where beta is estimated under
the scaled predictor envelope model.\end{verbatim}
...
\begin{verbatim}       TestOutput = testcoefficient_sxenv(ModelOutput)
       TestOutput = testcoefficient_sxenv(ModelOutput, TestInput)\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item TestInput.R: The matrix multiplied to $\beta$ on the right.  It is an r by d2 matrix, while d2 is less than or equal to r.  Default value: identity matrix $I_r$.
   \item TestInput.A: The matrix on the right hand side of the equation.  It is a d1 by d2 matrix.  Default value: d1 by d2 zero matrix.
\end{itemize}
...
\begin{par}
\textbf{TestOutput}: A list containing test statistics, degrees of freedom for the reference chi-squared distribution, the p-value, and the covariance matrix of vec(L\$\ensuremath{\backslash}beta\$R).  At the same time, a table is printed out.
\end{par} \vspace{1em}
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item TestOutput.chisqStatistic: The test statistics. A real number.
   \item TestOutput.df: The degrees of freedom of the reference chi-squared distribution.  A positive integer.
   \item TestOutput.pValue: p-value of the test.  A real number in [0, 1].
   \item TestOutput.covMatrix: The covariance matrix of vec(L$\hat{\beta}$R). A d1 * d2 by d1 * d2 matrix.
\end{itemize}
...
\begin{verbatim}       load('chemo.mat')
       ...
       TestInput.A = zeros(2, 1);
       TestOutout = testcoefficient_sxenv(ModelOutput, TestInput);\end{verbatim}
...
\rmfamily
\color{black}\section{mfoldcv\_xenv}
...
\begin{verbatim}       SelectOutput = mfoldcv_xenv(X, Y, m)
       SelectOutput = mfoldcv_xenv(X, Y, m, Opts)\end{verbatim}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item SelectOutput.u: The dimension of the envelope subspace selected by m-fold cross validation.  An integer between 0 and p.
   \item SelectOutput.PreErr: A vector containing prediction errors for each u if Opts.perm is not specified, or a matrix with the element in the ith row and jth column containing the prediction error for u=j-1 and ith permutation of the observations.
...
\begin{verbatim}       load wheatprotein.txt
       ...
       Y = wheatprotein(:, 7);
       Opts.table = 1; % Print out the table of average prediction error for each u
       SelectOutput = mfoldcv_xenv(X, Y, 5, Opts);\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.gradtol: Tolerance parameter for dF.  Default value: 1e-7.
   \item Opts.verbose: Flag to print out  Grassmann manifold optimization process, logical 0 or 1. Default value: 0.
      \item Opts.init: The initial value for the envelope subspace. A p by u matrix. Default value is the one generated by function get\_Init.
\end{itemize}
...
\begin{itemize}
\setlength{\itemsep}{-1ex}
   \item ModelOutput.beta: The envelope estimator of the regression coefficients $\beta$. A p by r matrix.
   \item ModelOutput.SigX: The envelope estimator of the covariance matrix of X, $\Sigma_X$.  A p by p matrix.
   \item ModelOutput.Gamma: The orthogonal basis of the envelope subspace. A p by u semi-orthogonal matrix.
   \item ModelOutput.Gamma0: The orthogonal basis of the complement of the envelope subspace.  A p by p-u semi-orthogonal matrix.
   \item ModelOutput.eta: The coordinates of $\beta$ with respect to Gamma. A u by r matrix.
   \item ModelOutput.Omega: The coordinates of $\Sigma_X$ with respect to Gamma. A u by u matrix.
   ...
   \item ModelOutput.ratio: The asymptotic standard error ratio of the standard multivariate linear regression estimator over the envelope estimator, for each element in $\beta$.  A p by r matrix.
   \item ModelOutput.paramNum: The number of parameters in the envelope model.  A positive integer.
   \item ModelOutput.n: The number of observations in the data.  A positive integer.
\end{itemize}
...
\begin{enumerate}
\setlength{\itemsep}{-1ex}
   \item The codes are implemented based on the algorithm in Section 4.5.1 of Cook et al (2013).
   \item The Grassmann manifold optimization step calls the package sg\_min 2.4.3 by Ross Lippert (http://web.mit.edu/$\sim$ripper/www/sgmin.html).
\end{enumerate}
...
\begin{verbatim}       SelectOutput = mfoldcv_xenvpls(X, Y, m)
       SelectOutput = mfoldcv_xenvpls(X, Y, m, Opts)\end{verbatim}    
...
\begin{verbatim}       load VocabGrowth
       ...
       Y = VocabGrowth(:, 4);
       Opts.table = 1; % Print out the table of average prediction error for each u
       SelectOutput = mfoldcv_xenvpls(X, Y, 5, Opts);\end{verbatim}
...
\setlength{\itemsep}{-1ex}
   ...
   \item ModelOutput.asySE: The asymptotic standard error for elements in $\beta$.  An r by p matrix.  The standard errors returned are asymptotic, for actual standard errors, multiply by 1/sqrt(n).
   \item ModelOutput.n: The number of observations in the data.  A positive integer.
\end{itemize}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.verbose: Flag to print out  output, logical 0 or 1.
\end{itemize}
...
\setlength{\itemsep}{-1ex}
   ...
   \item Opts.ftol: Tolerance parameter for F.  Default value: 1e-10.
   \item Opts.gradtol: Tolerance parameter for dF.  Default value: 1e-7.
   \item Opts.verbose: Flag to print out  output, logical 0 or 1. Default value: 0.
\end{itemize}
...
\end{par} \vspace{1em}
\begin{par}
\textbf{method}: A string of characters indicating which member of the envelope family to be used, the choices can be 'env', 'ienv', 'henv', 'senv', 'sxenv', 'xenv' or 'xenvpls'.
\end{par} \vspace{1em}
...
\setlength{\itemsep}{-1ex}
   ...
   \item DataParameter.betaOLS: The regression coefficients from the ordinary least squares regression of Y on X.  An r by p matrix.  For all methods except 'henv'.
   \item DataParameter.invsigY: The inverse of the sample covariance matrix of Y.  An r by r matrix.  For all methods except 'ienv'.
    \item DataParameter.invsigRes: The inverse of the sample covariance matrix of the residuals form the ordinary least squares regression of Y on X.  An r by r matrix. Only for method 'ienv'.
\end{itemize}
