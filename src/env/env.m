...
% * Opts.verbose: Flag for printing out Grassmann manifold optimization 
% process, logical 0 or 1. Default value: 0.
% * Opts.init: The initial value for the envelope subspace. An r by u matrix. Default
% value is the one generated by function get_Init. 
% * Opts.initscale: Flag for using scaled or unscaled initial values
% discussed in Cook, Forzani and Su (2015).  If the flag is 0, only 
% unscaled starting values will be calculated, if the flag is 1, both 
% scaled and unscaled starting values will be calculated. Default value: 1.
%
%% Output
% 
...
if u > 0 && u < r
    ...
    covMatrix = kron(inv(sigX), Sigma1) + kron(eta', Gamma0) / temp * kron(eta, Gamma0');
